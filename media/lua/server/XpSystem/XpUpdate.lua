...
xpUpdate.getFitnessModifier = function(player)
	-- gets the size of the regularity bar (to be divided by with to get percentage)
	local squatRegularity = (player:getFitness():getRegularity("squats") or 0) * 2; -- adds 2 to total modifier
	local situpRegularity = (player:getFitness():getRegularity("situp") or 0) * 2; -- adds 2 to total modifier
	local burpeeRegularity = (player:getFitness():getRegularity("burpees") or 0); -- adds 1 to total modifier
	
	return (squatRegularity / 100.0) + (situpRegularity / 100.0) + (burpeeRegularity / 100.0);
end

xpUpdate.getStrengthModifier = function(player)
	local pushupRegularity = (player:getFitness():getRegularity("pushups") or 0) * 2; -- adds 2 to total modifier

	local burpeeRegularity = player:getFitness():getRegularity("burpees") or 0; -- adds 1 to total modifier
	local bbCurlRegularity = player:getFitness():getRegularity("barbellcurl") or 0; -- adds 1 to total modifier
	local dbPressRegularity = player:getFitness():getRegularity("dumbbellpress") or 0; -- adds 1 to total modifier
	local dbCurlRegularity = player:getFitnesss():getRegularity("bicepscurl") or 0; -- adds 1 to total modifier

	return (pushupRegularity / 100.0) + (burpeeRegularity / 100.0) + (bbCurlRegularity / 100.0) + (dbPressRegularity / 100.0) + (dbCurlRegularity / 100.0);
end
...
xpUpdate.onPlayerMove = function()
	...
	if (player:IsRunning() or player:isSprinting()) and player:getStats():getEndurance() > player:getStats():getEndurancewarn() then
		...
		if xpUpdate.randXp() then
			xp:AddXP(Perks.Fitness, 1 + xpUpdate.getFitnessModifier(player));
		end
		if xpUpdate.randXp() then
	...
	if player:getInventoryWeight() > player:getMaxWeight() * 0.5 then
		if xpUpdate.randXp() then
			xp:AddXP(Perks.Strength, 2 + xpUpdate.getStrengthModifier(player));
		end
	end
end
